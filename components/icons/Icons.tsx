...
const sizeMap = { 4: "h-4 w-4", 5: "h-5 w-5", 6: "h-6 w-6" };

export function Icon({ name, className = "", size = 5 }: IconProps) {
  const sizeCls = sizeMap[size as keyof typeof sizeMap] ?? "h-5 w-5";
  const cls = `shrink-0 ${sizeCls} ${className}`;
